...
def deduction(noeud1, noeud2, relationName):
    relationNum = numsRelations.get(relationName)

    # récuérer tous les génériques de noeud1
    urlGenNoeud1 = f"https://jdm-api.demo.lirmm.fr/v0/relations/from/{noeud1}?types_ids=6&min_weight=1"
    ...
    if len(dataGenN1["relations"]) == 0:
        print(f"Non, pas selon JDM || {noeud1} n'a pas de générique")
        return 0
    ...
    # Normaliser (même relation même départ)
    maxW = 0
    ...
    if not intersectionIDs:
        print(f"Non || Aucune intersection entre les génériques de {noeud1} et les noeuds ayant une relation de type {relationName} vers {noeud2}")
        return 0
    ...
    for idNoeud in sortedIntersectionIDs.keys(): 
        for noeud in dataGenN1["nodes"]:
            if noeud["id"] == idNoeud:
                print(f"{i} || oui || {noeud1} -r_isa-> {noeud["name"]}  >  {noeud["name"]} -{relationName}-> {noeud2} || poids = {intersectionIDs[idNoeud]}")
                i+=1
        if i > 10:
            break
...
# Réponse = OUI
deduction("pigeon","voler","r_agent-1")
#deduction("tigre","chasser","r_agent-1")
#deduction("tigre","dangereux","r_carac")

# Réponse = NON
#deduction("couper","poulet","r_patient")

#deduction("Airbus A380","attérir","r_agent-1")
#deduction("avocat","pousser","r_agent-1")
#deduction("pigeon","pondre","r_agent-1")
#deduction("Tour Eiffel","France","r_lieu")
#deduction("chat","miauler","r_agent-1")
#deduction("oiseau","voler","r_agent-1")
#deduction("serveuse","apporter à boire","r_agent-1")
