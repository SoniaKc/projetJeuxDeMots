...
def transitivite(noeud1, noeud2, relationName):
    
    relationNum = numsRelations.get(relationName)

    # récuérer tous les noeuds accessibles avec relationNum depuis noeud1
    urlGenNoeud1 = f"https://jdm-api.demo.lirmm.fr/v0/relations/from/{noeud1}?types_ids={relationNum}&min_weight=1"
    # récupérer tous les noeuds allant vers noeud2 avec relationNum
    ...
    if len(dataGenN1["relations"]) == 0:
        print(f"Non, pas selon JDM || il n'y a pas de relation de type {relationNum} depuis {noeud1}")
        return 0
    ...
    if not intersectionIDs:
        print(f"Non || Aucune intersection par transitivité entre {noeud1} et {noeud2} avec la relation de type {relationName}")
        return 0
    ...
    for idNoeud in sortedIntersectionIDs.keys(): 
        for noeud in dataGenN1["nodes"]:
            if noeud["id"] == idNoeud:
                print(f"{i} || oui || {noeud1} -{relationName}-> {noeud["name"]}  >  {noeud["name"]} -{relationName}-> {noeud2} || poids = {intersectionIDs[idNoeud]}")
                i+=1
        if i > 10:
            break
...
# R2ponse = OUI
transitivite("piston","voiture","r_holo")
#transitivite("Tour Eiffel","France","r_lieu")
#transitivite("cellule","humain","r_holo")


#transitivite("avocat","pousser","r_agent-1")
#transitivite("tigre","chasser","r_agent-1")
#transitivite("pigeon","voler","r_agent-1")
#transitivite("chat","dangereux","r_carac")
#transitivite("chat","miauler","r_agent-1")
#transitivite("serveuse","apporter à boire","r_agent-1")
#transitivite("couper","poulet","r_patient")
